...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
 ...
 $time_buy      = check_string($_POST['time_buy']);
   ...
   $server_buy   = check_string($_POST['server']);
    ...
    if($action=="mem-group"){$link_post="member-group";}
    elseif($action=="like-post"){$link_post="like-post-speed";}
    elseif($action=="like-post-sale"){$link_post="like-post-sale";}

    elseif($action=="sub-sale"){$link_post="sub-sale";}
    elseif($action=="sub-speed"){$link_post='sub-speed';}
    elseif($action=="sub-vip"){$link_post='sub-vip';}
      elseif($action=="sub-v2"){$link_post='follow-v2';}
        elseif($action=="sub-v3"){$link_post='subv3';}
    elseif($action=="sub-quality"){$link_post='sub-quality';}
    elseif($action=="cmt"){$link_post="comment-sale";}
   
    elseif($action=="like-page-sale"){$link_post="like-page-sale";}
    elseif($action=="like-page-speed"){$link_post="like-page-speed";}
    elseif($action=="like-page-quality"){$link_post="like-page-quality";}
    elseif($action=="share"){$link_post="share-profile";}
    elseif($action=="share-sale"){$link_post="share-sale";}
    elseif($action=="mat-live"){$link_post="eye-live";}
    elseif($action=="view-video"){$link_post="view-video";}
    elseif($action=="like-cmt"){$link_post="like-comment";}
    elseif($action=="view-story"){$link_post="view-story";}
...
$url_POST="https://subgiare.me/function/api.php?type=facebook&key=$token_auto_dvfb&idpost=$id&server=sv$server_buy&amount=$soluong&note=$ghichu&ma=$link_post&reaction=$loai_camxuc&speed=$speed&comment=".urlencode($list_msg)."&minutes=$time_buy";
...
$url_post="https://$mien_site_me_thuong/auth/service/facebook/site_con.php?id=$id&soluong=$soluong&server=$server_buy&code_dich_vu=$code_dich_vu&action=$action&loai_camxuc=$loai_camxuc&time_buy=$time_buy&list_msg=$list_msg&domain_site_con=$mien_site_con&domain_site_me=$mien_site_me";
$ch = curl_init(); 
