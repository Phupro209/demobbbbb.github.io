...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
   ...
   $server_buy   = check_string($_POST['server']); $ghichu   = check_string($_POST['ghichu']);
    ...
    $api_token =  check_string($_POST['api_token']);
    if(empty($api_token)){
         $return['status'] = false;
        $return['error'] = true;
...
elseif($action=="like"){
    $link_post="like-tiktok";
    
}
elseif($action=="follow"){
    $link_post="sub-tiktok";
}
elseif($action=="view"){
    $link_post="view-tiktok";
}
       if($action=="cmt"){
...
");
    if(!$username_khach){
      ...
      $gia_dich_vu = $NTD->get_row("SELECT * FROM `server_service` WHERE `code_service` = '$code_dich_vu' AND `status_service` = '1' AND `server_service` = '$server_buy'");
       $server_name=$gia_dich_vu['server_text'];
...
$url_POST="https://subgiare.me/function/api.php?type=facebook&key=$token_auto_dvfb&idpost=$id&server=$server_name&amount=$soluong&note=$ghichu&ma=$link_post&reaction=$loai_camxuc&speed=$speed&comment=".urlencode($list_msg)."&minutes=$time_buy";
      ...
      elseif($username_admin['capbac']==99){
        ...
        $ratesv_site_me= $gia_dich_vu_site_me['rate_service']+$gia_dich_vu_site_me['rate_service']*$chiet_khau_auto/100;
         ...
         if($action=="sub-sale"){
          ...
          $url="https://graph.tiktok.com/$id/subscribers?summary=true&fields=id&limit=1&access_token=$token_fb";
  $ch = curl_init();
