...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
   ...
   if($_POST['action']=="change-token"){
    ...
    if(empty($api_token)){
        ...
        if($update2 = $NTD->update("users", ['token' => "eyJhbGciO".(random('qwertyuiopasdfghjklzxcvbnmQWERTYUIOPghjklzxcvbnmQWERTYUIOPghjklzxcvbnmQWERTYUIOPghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM', 100).time()).typepass2(random('qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890', 100).time())], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."' ")){
            $return['status'] = true;
    ...
    if($update2 = $NTD->update("users", ['password' => typepass($new_password)], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."'")){
        $return['status'] = true;
        $return['error'] = false;
    ...
    if($type == 'collaborators'){
        if($checkusite['tongnap'] < $NTD->site('rate_ctv')){
            ...
            $update1 = $NTD->update("users", ['capbac' => 1], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."'");
    ...
    if($type == 'agency'){
        if($checkusite['tongnap'] < $NTD->site('rate_daily')){
            ...
            $update2 = $NTD->update("users", ['capbac' => 2], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."'");
        ...
        if($checkusite['tongnap'] < $NTD->site('rate_admin')){
            ...
            $update1 = $NTD->update("users", ['capbac' => 4], " `username` = '".$checkusite['username']."' AND  `url_config` = '".$url_site_name."'");
