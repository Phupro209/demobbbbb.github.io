...
if ( $_SERVER['REQUEST_METHOD'] == 'GET' && realpath(__FILE__) == realpath( $_SERVER['SCRIPT_FILENAME'] ) ) {
 ...
 $gia_sub_sale = $NTD->get_row("SELECT * FROM `server_service` WHERE `code_service` = 'sub_sale' AND `status_service` = '1' AND `server_service` = '$so' AND  `url_config` = '".$get_site2['site_me']."' ");           
    ...
    $gia_sub_v2 = $NTD->get_row("SELECT * FROM `server_service` WHERE `code_service` = 'sub_v2' AND `status_service` = '1' AND `server_service` = '$so' AND  `url_config` = '".$get_site2['site_me']."' ");           
       $chiet_khau_sub_v2=$gia_sub_v2['rate_service']+$gia_sub_v2['rate_service']*$chiet_khau_auto/100;
     
     
     $insert_sub_v2= $NTD->insert("server_service", [
      'code_service'      => $gia_sub_v2['code_service'],
      'server_service'          =>$gia_sub_v2['server_service'],
      'rate_service'      => $chiet_khau_sub_v2,
       'title_service'         => $gia_sub_v2['title_service'],
       'status_service'        => 1,
       'server_name'=>$gia_sub_v2['server_name'],
        'server_text'=>$gia_sub_v2['server_text'],
      'url_config'          => $url_site_name,
                                    ]);                                       
                                    
                                    
      $gia_sub_v3 = $NTD->get_row("SELECT * FROM `server_service` WHERE `code_service` = 'sub_v3' AND `status_service` = '1' AND `server_service` = '$so' AND  `url_config` = '".$get_site2['site_me']."' ");           
       $chiet_khau_sub_v3=$gia_sub_v3['rate_service']+$gia_sub_v3['rate_service']*$chiet_khau_auto/100;
     
     
     $insert_sub_v3= $NTD->insert("server_service", [
      'code_service'      => $gia_sub_v3['code_service'],
      'server_service'          =>$gia_sub_v3['server_service'],
      'rate_service'      => $chiet_khau_sub_v3,
       'title_service'         => $gia_sub_v3['title_service'],
       'status_service'        => 1,
       'server_name'=>$gia_sub_v3['server_name'],
        'server_text'=>$gia_sub_v3['server_text'],
      'url_config'          => $url_site_name,
                                    ]);                                     
