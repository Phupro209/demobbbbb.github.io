...
console.log(
    "%c Telegram: @nddvpbq %c",
    'font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;font-size:24px;color:#ff0011;-webkit-text-fill-color:#ff0011;-webkit-text-stroke: 1px #ff0011;',
    "font-size:12px;color:#999999;"
);
console.log(
    "%c Facebook: https://www.facebook.com/Nguyen.Duc.Duy.Vo.Dich.Thien.Ha %c",
    'font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;font-size:24px;color:#ff0011;-webkit-text-fill-color:#ff0011;-webkit-text-stroke: 1px #ff0011;',
    "font-size:12px;color:#999999;"
);
console.log(
...
$(document).ready(function () {
    ...
    $("form[submit-ajax=NTD]").submit(function (e) {
     ...
     $("form[submit-ajax=NTIENDAT]").submit(function (e) {
        e.preventDefault();
        let _this = this;
        let url = $(_this).attr("action");
        let method = $(_this).attr("method");
        ...
        let data = $(_this).serialize();
        let button = $(_this).find("button[type=submit]");
        if (button.attr("xacnhan")) {
            Swal.fire({
                title: "Xác nhận đăng kí !",
                text: button.attr("xacnhan"),
                icon: "warning",
                showCancelButton: true,
                // confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    Swal.fire({
                        icon: "warning",
                        title: "Đang tiến hành tạo website, vui lòng không F5, quá trình sẽ mất 1 - 3 phút !",
                        timer: 180000,
                        timerProgressBar: true,
                        showCancelButton: false,
                        showConfirmButton: false,
