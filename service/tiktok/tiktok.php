...
elseif($dich_vu=="view"){$dtb="view_tiktok"; $action=$dichvu1['5']; $title="View TikTok";}
...
<label for="" class="form-label col-md-3"><?php if($action=="follow"){?>Username<?php }else{?>Link video <?php }?></label>
<div class="col-md-9">
<input <?php if($action=="follow"){?> type="text" placeholder="Nhập username cần tăng hoặc nhập link hệ thống tự get user" <?php }else{?>type="url" placeholder="Nhập link video cần tăng"<?php }?> class="form-control" name="id" <?php if($action=="follow"){?>onchange="ttGetUsername('id');"<?php }else{?>onchange="bill();"<?php }?>  >
</div>
</div>
...
&nbsp;(<?=$showrate['title_service']?>)&nbsp;<span class="badge bg-success text-white "><?php if($lam_tron=="ON"){echo ceil($showrate['rate_service']+($showrate['rate_service']*$chietkhau/100));}else{echo $showrate['rate_service']+($showrate['rate_service']*$chietkhau/100);}?> đ / lượt</span>&nbsp;<b class="text-warning">(Hoạt đông)</b></label>
                </div>
            </div>
        </div> <?php }?>
