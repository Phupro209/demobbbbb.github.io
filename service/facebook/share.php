...
&nbsp;(<?=$showrate['title_service']?>)&nbsp;<span class="badge bg-success text-white "><?php if($lam_tron=="ON"){echo ceil($showrate['rate_service']+($showrate['rate_service']*$chietkhau/100));}else{echo $showrate['rate_service']+($showrate['rate_service']*$chietkhau/100);}?> đ / share</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                </div>
            </div>
        </div> <?php }?>
...
<div class="alert bg-danger text-white" role="alert"><h4>Vui lòng đọc tránh mất tiền</h4>- <b>Mua bằng ID Page đã mở chế độ công khai, có nút like</b>.<br>- <b>Không hỗ trợ tăng cho pro5, cố tính tăng sẽ không hỗ trợ</b>.</div> </div>
</div>
<div class="form-group row mb-3">
