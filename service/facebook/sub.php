...
elseif($dich_vu=="sub-quality"){$dtb="sub_quality"; $action=$dichvu1['5']; $title="Follow Quality";}elseif($dich_vu=="sub-v2"){$dtb="sub_v2"; $action=$dichvu1['5']; $title="Follow V2";}
...
<div class="col-md-9">
<input class="form-control" name="id"  placeholder="Nhập id hoặc nhập link profile" onchange="getUID('id');" >
...
&nbsp;(<?=$showrate['title_service']?>)&nbsp;<span class="badge bg-success text-white "><?php if($lam_tron=="ON"){echo ceil($showrate['rate_service']+($showrate['rate_service']*$chietkhau/100));}else{echo $showrate['rate_service']+($showrate['rate_service']*$chietkhau/100);}?> đ / sub</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                </div>
            </div>
        </div> <?php }?>
