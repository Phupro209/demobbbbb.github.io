    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header bg-transparent">
                <h3 class="mb-0">Cài đặt mini game Tài xỉu</h3>
            </div>
            <div class="card-body">
     <?php
     $GET_GAME = $VIP->get_row(" SELECT * FROM `danh_sach_game` WHERE `ma_game` = 'tai-xiu'  ");
     
     $GET_GAME_CONFIG_1 = $VIP->get_row(" SELECT * FROM `settings_game` WHERE `key` = 'tai-xiu' AND  `phan_game` = 'comment_1' ");
     $GET_GAME_CONFIG_2 = $VIP->get_row(" SELECT * FROM `settings_game` WHERE `key` = 'tai-xiu' AND  `phan_game` = 'comment_2' ");
     ...
     ?>
                ...
                <input type="hidden" name="_token" value="tVjXfr7sEdKf63XpMdYuGZ7wm90Bfptbq3YEqdvw">
                  <input type="hidden" name="action" value="setting_games">              <input name="service" type="hidden" value="tai-xiu">
