                        ...
                        <div class="card card-stats">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-0 text-sm">
                                    <span class="text-nowrap d-block">Tổng nhận: <?=format_money($VIP->get_row("SELECT SUM(`amount_play`) FROM `lich_su_choi` WHERE `result_text` != 'SAI NỘI DUNG' AND `time` >= DATE(NOW()) AND `time` < DATE(NOW()) + INTERVAL 1 DAY ")['SUM(`amount_play`)']);?>đ</span>
                                    <span class="text-nowrap d-block">Tổng trừ: <?=format_money($VIP->get_row("SELECT SUM(`amount`) FROM `chuyen_tien` WHERE `type_gd` = 'game' AND `date_time` >= DATE(NOW()) AND `date_time` < DATE(NOW()) + INTERVAL 1 DAY ")['SUM(`amount`)']);?>đ</span>
                                    <span class="text-nowrap d-block">Doanh thu: <?=format_money($VIP->get_row("SELECT SUM(`amount_play`) FROM `lich_su_choi` WHERE  `result_text` != 'SAI NỘI DUNG' AND `time` >= DATE(NOW()) AND `time` < DATE(NOW()) + INTERVAL 1 DAY ")['SUM(`amount_play`)']- $VIP->get_row("SELECT SUM(`amount`) FROM `chuyen_tien` WHERE  `type_gd` = 'game' AND `date_time` >= DATE(NOW()) AND `date_time` < DATE(NOW()) + INTERVAL 1 DAY ")['SUM(`amount`)']);?>đ</span>
                                </div>
                            </div>
                        </div>
                    </div>
