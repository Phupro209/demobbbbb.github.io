    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header bg-transparent">
                <h3 class="mb-0">Cài đặt mini game Chẵn Lẻ 2</h3>
            </div>
            <div class="card-body">
     <?php
     $GET_GAME = $VIP->get_row(" SELECT * FROM `danh_sach_game` WHERE `ma_game` = 'chan-le-2'  ");
     
     $GET_GAME_CONFIG_1 = $VIP->get_row(" SELECT * FROM `settings_game` WHERE `key` = 'chan-le-2' AND  `phan_game` = 'comment_1' ");
     $GET_GAME_CONFIG_2 = $VIP->get_row(" SELECT * FROM `settings_game` WHERE `key` = 'chan-le-2' AND  `phan_game` = 'comment_2' ");
     ...
     ?>
                ...
                <input type="hidden" name="_token" value="tVjXfr7sEdKf63XpMdYuGZ7wm90Bfptbq3YEqdvw">
                  <input type="hidden" name="action" value="setting_games">              <input name="service" type="hidden" value="chan-le-2">
              <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="example-text-input" class="form-control-label">Nội Dung Chẵn</label>
                        <input class="form-control" name="comment_1" type="text" value="<?=$GET_GAME_CONFIG_1['comment'];?>" placeholder="Nội Dung Chẵn">
                    </div>
                    <div class="form-group">
                        <label for="example-text-input" class="form-control-label">Nội Dung Lẻ</label>
                        <input class="form-control" name="comment_2" type="text" value="<?=$GET_GAME_CONFIG_2['comment'];?>" placeholder="Nội Dung Lẻ">
                    </div>
                    <div class="form-group">
