<?php
require_once(realpath($_SERVER["DOCUMENT_ROOT"]) .'/core/@connect.php');

//header("content-type: application/json;charset=utf-8");
$momo = new MOMO($VIP->connect());
...
class MOMO
{
    ...
    public function ImportOTP($code)
    {
        ...
        if(!empty($result["errorCode"])){
            ...
            $this->connect->query("UPDATE `cron_momo` SET z`status` = 'error',`errorDesc` = 'Hết thời gian truy cập vui lòng đăng nhập lại' WHERE `phone` = '".$this->config["phone"]."' ");
            return array(
                "status" => "error",
                "code"   => -5,
                "message"=> "Hết thời gian truy cập vui lòng đăng nhập lại"
   ...
   public function CheckHistory($day)
    {
        $resuts = $this->QUERY_TRAN_HIS_MSG_NEWV2($day);
        // die(print_r($resuts));
    ...
    public function QUERY_TRAN_HIS_MSG_NEWV2($day = 30)
    {
      ...
      public function QUERY_TRAN_HIS_MSG_NEW($day)
    {
        $begin =  (time() - (3600 * $day)) * 1000;
        $header = array(
            "authorization: Bearer " . $this->config["authorization"],
            "user_phone: " . $this->config['phone'],
            "Host: m.mservice.io",
       ...
       if(!is_array($result)){
        ...
        if($tranHisMsg){
          $return = array();
        foreach ($tranHisMsg as $value){
            if ($value['type'] == 77 or $value['type'] == 90) {
            //$extra = json_decode($value["extra"],true);
            $amount = $value['caption'];
            $name = explode("từ", $amount) [1] ? : "";
            if (strpos($amount, "Nhận") !== false && $name)
